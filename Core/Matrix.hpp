	...
	namespace Data
	{
		...
		public:
				...
				: Matrix(matrix.row, matrix.col)
			{
				...
				std::memcpy(data.get(), matrix.data.get(), sizeof(T) * elementCount);
			}
			...
			{
				return Core::Helper::MatrixHelper::ElementWiseEqual(this->data.get(), elementCount * sizeof(T), matrix.data.get(), matrix.elementCount * sizeof(T));
			}
		...
		private:
			...
			void copy(const Matrix<T> &_m)
			{
				...
				// if (data != nullptr)
				// {
				// 	data.reset(new[elementCount] T);
				// }
				// std::memcpy(data.get(), _m.data.get(), elementCount * sizeof(T));
			}
		};
