...
public:
    net_tree(hash_fn_t<k_arg> hash_func = hash_in_built) :
        p_hash_fn(hash_func) {}
    net_tree(init_list_kv_t<k_arg, arg> init_list, hash_fn_t<k_arg> hash_func = hash_in_built) : net_tree(hash_func) { insert(init_list); }
    ...
    arg &get_value(uint64_t hash_key) const {
        ...
        if (!tgt_leaf || tgt_leaf->hash_key != hash_key) return neunet_null_ref(arg);
